...
func CreateSettingsToRegedit(settingsName string, settingValue string) {
	k, err := registry.OpenKey(registry.CURRENT_USER, `Software\RaindropsVC`, registry.ALL_ACCESS)
	if err != nil {
		k, _, err = registry.CreateKey(registry.CURRENT_USER, `Software\RaindropsVC`, registry.ALL_ACCESS)
		if err != nil {
			return
		}
	}
