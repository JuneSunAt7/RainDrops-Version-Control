...
func GetKeeps(repoName string) error {
	...
	gettingListOfKeeps(connect, repoName)
	connect.Write([]byte("close\n"))
	return nil
}
func gettingListOfKeeps(conn net.Conn, repoName string){
	conn.Write([]byte(fmt.Sprintf("get_repo %s\n", repoName)))
